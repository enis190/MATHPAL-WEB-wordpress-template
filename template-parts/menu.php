...
function is_menu_item_active($item, $menu_items_by_parent, $post_id) {
    ...
    if (!empty($menu_items_by_parent[$item->ID])) {
        foreach ($menu_items_by_parent[$item->ID] as $child) {
            if (is_menu_item_active($child, $menu_items_by_parent, $post_id)) {
                return true;
            }
        }
    }
...
function render_menu_items($parent_id, $menu_items_by_parent, $post_id) {
    ...
    foreach ($menu_items_by_parent[$parent_id] as $item) {
        ...
        $is_active = is_menu_item_active($item, $menu_items_by_parent, $post_id);
        ...
        if ($has_children) {
            render_menu_items($item->ID, $menu_items_by_parent, $post_id);
        }
...
<?php render_menu_items(0, $menu_items_by_parent, $post->ID); ?>
