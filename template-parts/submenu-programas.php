...
$menu      = ! empty( $locations['header-menu-2'] )
    ? wp_get_nav_menu_object( $locations['header-menu-2'] )
    : null;
...
?>
<div class="container-fluid submenu margin-menu-principal">
    <div class="container">
        <div class="row">
            <div class="col-12 submenu-programas__scroll">
                <?php
                //global $post;
                ...
                function is_menu_item_active2($item, $menu_items_by_parent, $post_id)
                {
                    ...
                    if (!empty($menu_items_by_parent[$item->ID])) {
                        foreach ($menu_items_by_parent[$item->ID] as $child) {
                            if (is_menu_item_active2($child, $menu_items_by_parent, $post_id)) {
                                return true;
                            }
                        }
                    }
                ...
                function render_menu_items2($parent_id, $menu_items_by_parent, $post_id)
                {
                    ...
                    foreach ($menu_items_by_parent[$parent_id] as $item) {
                        ...
                        $is_active = is_menu_item_active2($item, $menu_items_by_parent, $post_id);
                        ...
                        echo '<a href="' . esc_url($url) . '" class="main_link' . $active_class . '"><span>' . get_post_meta($item->object_id, 'icono', true) . ' ';
                        echo esc_html($title);
                        ...
                        if ($has_children) {
                            render_menu_items2($item->ID, $menu_items_by_parent, $post_id);
                        }
                ...
                <?php render_menu_items2(0, $menu_items_by_parent, $post->ID); ?>
